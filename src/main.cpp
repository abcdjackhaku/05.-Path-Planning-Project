...
vector<double> ConverttoFrenet(double x, double y, double theta, const vector<double> &maps_x, const vector<double> &maps_y)
{
	int dNextWp = NextWaypoint(x, y, theta, maps_x, maps_y);

	int dPrevWp;
	dPrevWp = dNextWp - 1;
	if (dPrevWp == 0)
	{
		dPrevWp = maps_x.size() - 1;
	}

	double n_x = maps_x[dNextWp] - maps_x[dPrevWp];
	double n_y = maps_y[dNextWp] - maps_y[dPrevWp];
	double x_x = x - maps_x[dPrevWp];
	double x_y = y - maps_y[dPrevWp];
	...
	double center_x = 1000 - maps_x[dPrevWp];
	double center_y = 2000 - maps_y[dPrevWp];
...
vector<double> ConverttoXY(double s, double d, const vector<double> &maps_s, const vector<double> &maps_x, const vector<double> &maps_y)
{
	int dPrevWp = -1;

	while (s > maps_s[dPrevWp + 1] && (dPrevWp < (int)(maps_s.size() - 1)))
	{
		dPrevWp++;
	}

	int wp2 = (dPrevWp + 1) % maps_x.size();

	double heading = atan2((maps_y[wp2] - maps_y[dPrevWp]), (maps_x[wp2] - maps_x[dPrevWp]));
	// the x,y,s along the segment
	double seg_s = (s - maps_s[dPrevWp]);

	double seg_x = maps_x[dPrevWp] + seg_s*cos(heading);
	double seg_y = maps_y[dPrevWp] + seg_s*sin(heading);
